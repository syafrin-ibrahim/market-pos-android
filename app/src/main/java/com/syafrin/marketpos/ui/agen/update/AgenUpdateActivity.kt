...
class AgenUpdateActivity : AppCompatActivity(), AgenUpdateContract.View {
    private var uriImage: Uri? = null

    private var pickImage = 1
    lateinit var presenter: AgenUpdatePresenter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_agen_create)
        presenter = AgenUpdatePresenter(this)
    ...
    override fun initListener() {
        ...
        btn_save.setOnClickListener {
            ...
            if(store.isNullOrEmpty() || pml.isNullOrEmpty() || almt.isNullOrEmpty()
                ...
                presenter.updateAgen(Constant.AGENT_ID, store.toString(), pml.toString(), almt.toString(),
    ...
    override fun onDetailResult(responseAgenDetail: ResponseAgenDetail) {
        val agent = responseAgenDetail.dataAgen
        edNamaToko.setText(agent.nama_toko.toString())
        edPemilik.setText(agent.nama_pemilik.toString())
        edAlamat.setText(agent.alamat.toString())
        edLokasi.setText("${agent.latitude}, ${agent.longitude}")
        Constant.LATITUDE = agent.latitude!!
        Constant.LONGITUDE = agent.longitude!!
         GlideHelper.setImage(this, agent.img_toko!!, imgToko)
    }

    override fun onUpdateResult(responseAgenUpdate: ResponseAgenUpdate) {
